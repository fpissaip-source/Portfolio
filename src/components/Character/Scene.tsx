...
import { updateScrollVariables } from "./scrollCss";
...
const Scene = () => {
  ...
  useEffect(() => {
    ...
    const loadingProgress = setProgress((value) => setLoading(value));
    ...
    const onTouchStart = (event: TouchEvent) => {
      ...
      touchDebounce = window.setTimeout(() => {
        element?.addEventListener("touchmove", (touchEvent: TouchEvent) => handleTouchMove(touchEvent, (x, y) => (mouse = { x, y })), { passive: true, once: true });
      }, 160);
    };
    ...
    const animate = () => {
      ...
      const progress = updateScrollVariables();
      if (character) {
        const mobile = isMobileViewport();
        const blend = Math.max(progress.intro * 0.7, progress.desk);
        character.rotation.y = blend * (mobile ? 0.82 : 0.92);
        ...
        if (headBone) headBone.rotation.x = progress.desk * (mobile ? 0.5 : 0.6);
      }
      if (headBone) {
  ...
  return <div className="character-container" aria-hidden="true"><div className="character-model" ref={canvasDiv}><div className="character-rim" /><div className="character-hover" ref={hoverDivRef} /></div></div>;
};
